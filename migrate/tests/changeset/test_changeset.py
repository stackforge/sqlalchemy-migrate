...
class TestColumnChange(fixture.DB):
    ...
    def _setup(self, url):
        ...
        self.table = Table(self.table_name, self.meta,
                           Column('id', Integer, primary_key=True),
                           Column(
                               'data', String(40), server_default=DefaultClause("tluafed"),
                               nullable=True),
                           )
        if self.table.exists():
            self.table.drop()
    ...
    def test_default(self):
        ...
        row = self.table.select(
            self.table.c.id == 11).execution_options(
            autocommit=True).execute(
        ).fetchone(
        )
...
class TestColumnDelta(fixture.DB):
    ...
    def test_deltas_zero_columns(self):
        ...
        self.meta.clear()
        delta = self.verify(
            ['type'], 'ids', table=self.table.name, type=String(80), metadata=self.meta,
            alter_metadata=True)
        ...
        self.meta.clear()
        self.verify(
            ['server_default'], 'ids', table=self.table.name, server_default='foobar',
            metadata=self.meta,
            alter_metadata=True)
