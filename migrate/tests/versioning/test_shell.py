...
from migrate.versioning import shell, api
...
class TestShellCommands(Shell):
    ...
    def test_main(self):
        ...
        self._check_error(
            ['create',
             ...
             'o'],
            2,
            'error: Too many arguments for command create: o')
        self._check_error(
            ['create'],
            2,
            'error: Not enough arguments for command create: name, '
    ...
    def test_script(self):
        ...
        self.assert_(
            os.path.exists(
                '%s/versions/003_Some_Random_name.py' %
                repos))
    ...
    def test_script_sql(self):
        ...
        self.assert_(
            os.path.exists(
                '%s/versions/001_foo_mydb_upgrade.sql' %
                repos))
        self.assert_(
            os.path.exists(
                '%s/versions/001_foo_mydb_downgrade.sql' %
                repos))

        # Test creating a second
        result = self.env.run(
            'migrate script_sql postgres foo --repository=%s' %
            repos)
        self.assert_(
            os.path.exists(
                '%s/versions/002_foo_postgres_upgrade.sql' %
                repos))
        self.assert_(
            os.path.exists(
                '%s/versions/002_foo_postgres_downgrade.sql' %
                repos))
...
class TestShellRepository(Shell):
    ...
    def setUp(self):
        ...
        result = self.env.run(
            'migrate create %s repository_name' %
            self.path_repos)
    ...
    def test_source(self):
        ...
        result = self.env.run(
            'migrate script Desc --repository=%s' %
            self.path_repos)
...
class TestShellDatabase(Shell, DB):
    ...
    # We'll need to clean up after ourself, since the shell creates its
    # own txn; we need to connect to the DB to see if things worked
    ...
    def test_version_control(self):
        ...
        result = self.env.run(
            'migrate version_control %(url)s %(repos)s' %
            locals())
    ...
    def test_wrapped_kwargs(self):
        ...
        url = self.url
        result = self.env.run(
            'migrate create --name=repository_name %s' %
            repos)
        result = self.env.run(
            'migrate drop_version_control %(url)s %(repos)s' %
            locals(), expect_error=True)
    ...
    def test_version_control_specified(self):
        ...
        result = self.env.run(
            ...
            path_repos)
        result = self.env.run(
            'migrate drop_version_control %(url)s %(path_repos)s' %
            locals(), expect_error=True)
        self.assertEqual(result.returncode, 1)
        ...
        result = self.env.run(
            'migrate db_version %(url)s %(path_repos)s' %
            locals())
        self.assertEqual(result.stdout.strip(), str(version))
    ...
    def test_upgrade(self):
        ...
        repos_path = self.tmp()
        result = self.env.run(
            'migrate create %(repos_path)s %(repos_name)s' %
            locals())
        self.assertEqual(self.run_version(repos_path), 0)

        # Version the DB
        result = self.env.run(
            'migrate drop_version_control %s %s' %
            (self.url, repos_path), expect_error=True)
        result = self.env.run(
            'migrate version_control %s %s' %
            (self.url, repos_path))
        ...
        # Test preview
        result = self.env.run(
            'migrate upgrade %s %s 0 --preview_sql' %
            (self.url, repos_path))
        result = self.env.run(
        ...
        result = self.env.run(
            'migrate downgrade %s %s' %
            (self.url, repos_path), expect_error=True)
        self.assertEqual(result.returncode, 2)
        result = self.env.run(
            'migrate downgrade %s %s -1' %
            (self.url, repos_path), expect_error=True)
        self.assertEqual(result.returncode, 2)
        result = self.env.run(
            'migrate downgrade %s %s 2' %
            (self.url, repos_path), expect_error=True)
        self.assertEqual(result.returncode, 2)
        self.assertEqual(self.run_db_version(self.url, repos_path), 1)

        result = self.env.run(
            'migrate downgrade %s %s 0' %
            (self.url, repos_path))
        self.assertEqual(self.run_db_version(self.url, repos_path), 0)
    ...
    def _run_test_sqlfile(self, upgrade_script, downgrade_script):
        ...
        beforeCount = len(os.listdir(os.path.join(repos_path, 'versions')))
        result = self.env.run(
            'migrate script_sql %s --repository=%s' %
            ('postgres', repos_path))
        self.assertEqual(self.run_version(repos_path), 1)
        self.assertEqual(
            len(os.listdir(os.path.join(repos_path, 'versions'))),
            beforeCount + 2)

        open(
            '%s/versions/001_postgres_upgrade.sql' %
            repos_path,
            'a').write(upgrade_script)
        open(
            '%s/versions/001_postgres_downgrade.sql' %
            repos_path,
    ...
    def test_command_test(self):
        ...
        result = self.env.run(
            'migrate create repository_name --repository=%s' %
            repos_path)
        result = self.env.run(
            'migrate drop_version_control %s %s' %
            (self.url, repos_path), expect_error=True)
        ...
        def upgrade():
            ...
            # use the engine named 'migrate_engine' imported from migrate.
            meta.create_all()
    ...
    def test_rundiffs_in_shell(self):
        # This is a variant of the test_schemadiff tests but run through the
        # shell level.
        # These shell tests are hard to debug (since they keep forking
        # processes)so they shouldn't replace the lower-level tests.
        repos_name = 'repos_name'
        repos_path = self.tmp()
        script_path = self.tmp_py()
        ...
        # Setup helper script.
        result = self.env.run('migrate manage %s --repository=%s --url=%s '
                              '--model=%s'
                              % (script_path, repos_path, self.url,
                                 model_module))
        ...
        self.assert_(
            ...
            "tmp_account_rundiffs" in result.stdout)

        # Test Deprecation
        result = self.env.run('migrate compare_model_to_db %s %s --model=%s'
                              % (self.url, repos_path,
                                 model_module.replace(":", ".")),
                              expect_error=True)
        ...
        result = self.env.run(
            'migrate create_model %s %s' %
            (self.url, repos_path))
        temp_dict = dict()
        exec result.stdout in temp_dict

        # TODO: breaks on SA06 and SA05 - in need of total refactor - use
        # different approach

        # TODO: compare whole table
        self.compare_columns_equal(
            models.tmp_account_rundiffs.c,
            temp_dict['tmp_account_rundiffs'].c,
            ['type'])
        # self.assertTrue("""tmp_account_rundiffs = Table('tmp_account_rundiffs', meta,    # noqa
  ##Column('id', Integer(),  primary_key=True, nullable=False),
  ##Column('login', String(length=None, convert_unicode=False, assert_unicode=None)),    # noqa
  # Column('passwd', String(length=None, convert_unicode=False,
  # assert_unicode=None))""" in result.stdout)

        # We're happy with db changes, make first db upgrade script to go from version 0 -> 1.    # noqa
        #result = self.env.run('migrate make_update_script_for_model', expect_error=True)    # noqa
        #self.assertTrue('Not enough arguments' in result.stderr)

        # result_script = self.env.run('migrate make_update_script_for_model %s %s %s %s'\    # noqa
        ...
        # tmp_account_rundiffs = Table('tmp_account_rundiffs', meta,
          #Column('id', Integer(),  primary_key=True, nullable=False),
          #Column('login', Text(length=None, convert_unicode=False, assert_unicode=None, unicode_error=None, _warn_on_bytestring=False)),    # noqa
          #Column('passwd', Text(length=None, convert_unicode=False, assert_unicode=None, unicode_error=None, _warn_on_bytestring=False)),    # noqa
        #)

        # def upgrade(migrate_engine):
            # Upgrade operations go here. Don't create your own engine; bind migrate_engine    # noqa
            # to your metadata
