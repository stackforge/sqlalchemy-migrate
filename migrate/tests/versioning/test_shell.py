...
class TestShellCommands(Shell):
    ...
    def test_main(self):
        ...
        self._check_error(
            ['create'],
            2,
            'error: Not enough arguments for command create: name, repository not specified')
        self._check_error(
            ['create',
             'repo_name'],
            2,
...
class TestShellDatabase(Shell, DB):
    ...
    def _run_test_sqlfile(self, upgrade_script, downgrade_script):
        ...
        self.assertEqual(self.run_version(repos_path), 1)
        self.assertEqual(
            len(os.listdir(os.path.join(repos_path, 'versions'))), beforeCount + 2)
    ...
    def test_rundiffs_in_shell(self):
        ...
        self.assert_(
            "tables missing from database: tmp_account_rundiffs" in result.stdout)
