...
def load_model(dotted_name):
    ...
    :param dotted_name: path to model in form of \
    string: ``some.python.module:Class``
...
def construct_engine(engine, **opts):
    ...
    Currently, there are 2 ways to pass create_engine options to \
    :mod:`migrate.versioning.api` functions:
    ...
    :param engine_arg_*: keyword parameters to pass to `create_engine` \
    (evaluated with :func:`migrate.versioning.util.guess_obj_type`)
    ...
    if isinstance(engine, Engine):
        ...
        raise ValueError(
            "you need to pass either an existing engine or a database uri")
